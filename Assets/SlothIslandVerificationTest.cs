...
public class SlothIslandVerificationTest : MonoBehaviour
{
    ...
    private const string ExpectedViceName = "Sloth";
    private const string ExpectedNextIslandId = "island_wrath";
    private const string SceneRelativePath = "Scenes/level_sloth.unity";

    [ContextMenu("Run Sloth Island Verification")]
    public void RunTests()
    {
        Debug.Log("=== Starting Sloth Island Verification ===");
        ...
        Debug.Log("=== Sloth Island Verification Passed ===");
    }
    ...
    private IslandConfig TestIslandConfigExists()
    {
        ...
        Assert.AreEqual(ExpectedViceName, config.viceName, "Sloth island should expose the correct vice name.");
        Assert.IsNotNull(config.encounters, "Sloth island should define an ordered encounter list.");
        return config;
    }
    ...
    {
        Assert.AreEqual(9, config.encounters.Length, "Sloth should have 9 ordered encounters (4 combat, 4 puzzle, 1 boss).");
        ...
        Assert.AreEqual(5, combat, "Sloth should have 5 combat encounters (4 standard plus the boss).");
        Assert.AreEqual(4, puzzle, "Sloth should have 4 puzzle encounters.");
    }
    ...
    private void TestRestorationBudget(IslandConfig config)
    {
        ...
        Assert.AreEqual(1f, total, 0.001f, "Sloth restoration budget should sum to 100%.");
    }
    ...
    private void TestBossEncounter(IslandConfig config)
    {
        ...
        Assert.IsTrue(!string.IsNullOrEmpty(boss.encounterId)
            ...
            "Sloth's final encounter should be the boss (its id should contain 'boss').");
        Assert.AreEqual(EncounterType.Combat, boss.type, "Sloth boss should be a combat encounter.");
        Assert.AreEqual(0.25f, boss.restorationValue, 0.001f, "Sloth boss should contribute 25% restoration.");
        Assert.IsNotNull(boss.encounterConfig, "Sloth boss should reference an EncounterConfig.");
        Assert.Greater(boss.encounterConfig.EnemyCount, 0, "Sloth boss encounter should include at least one enemy.");
    }
    ...
    private void TestPuzzleEncounters(IslandConfig config)
    {
        ...
        Assert.AreEqual(4, puzzleCount, "Sloth should contain 4 puzzle encounters.");
    }
    ...
    private void TestSceneExists()
    {
        ...
        {
            Debug.LogWarning($"[SlothIslandVerificationTest] Scene Assets/{SceneRelativePath} not found; finalize environment dressing in the Unity Editor.");
        }
    }
}
